...
int test_vector()
{
    const ncontainer* vector_kutori_type = nephren::get<Vector<kutori>>();
    Vector<kutori> kutori_vector = { kutori("kutori0"), kutori("kutori1") };
    ...
    {
        NTR_TEST_THROW(value.as<kutori>().get_name() == (*kutori_vector_it).get_name());
    ...
    NTR_TEST_ASSERT(kutori_construct + kutori_copy_construct + kutori_move_construct ==
                    kutori_destroy);
...
int test_map()
{
    const ncontainer* map_string_kutori_type = nephren::get<Map<std::string, kutori>>();
    Map<std::string, kutori> kutori_map;
    kutori_map.insert_or_assign("kutori0", kutori("kutori0"));
    kutori_map.insert_or_assign("kutori1", kutori("kutori1"));
    map_string_kutori_type->put(kutori_map, std::string("kutori2"), kutori("kutori2"));
    map_string_kutori_type->put(kutori_map, std::string("kutori3"), kutori("kutori3"));
    map_string_kutori_type->put(kutori_map, std::string("kutori0"), kutori("kutori4"));
        ...
        kutori_map, [&kutori_map, &kutori_map_it](nobject&& key, nobject&& value)
    {
        ...
        NTR_TEST_THROW(&key.as<std::string>() == &kutori_map_it->first);
        NTR_TEST_THROW(value.as<kutori>().get_name() == kutori_map_it->second.get_name());
...
int test_set()
{
    const ncontainer* set_float_type = nephren::get<Set<float>>();
    Set<float> float_set;
...
int main()
{
    try
    {
        ...
        std::map<int, int>::value_type int_map_value;
        if (test_vector<nvector>() != 0)
            return 1;
        if (test_vector<std::vector>() != 0)
            return 1;
        if (test_map<nhash_map>() != 0)
            return 1;
        if (test_map<std::map>() != 0)
            return 1;
        if (test_set<nhash_set>() != 0)
            return 1;
        if (test_set<nhash_set>() != 0)
            return 1;
        if (test_set<std::set>() != 0)
            return 1;
        return 0;
    }

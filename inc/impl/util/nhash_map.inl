...
nhash_map_table_traits<Key, Value>::get_key(const value_type& value)
{
    return value.first;
}
...
template <class Key, class Value, class Hash, class Allocator>
NTR_INLINE typename nhash_map<Key, Value, Hash, Allocator>::mapped_type&
...
template <class Key, class Value, class Hash, class Allocator>
NTR_INLINE const typename nhash_map<Key, Value, Hash, Allocator>::mapped_type&
...
nhash_map<Key, Value, Hash, Allocator>::operator[](const key_type& key)
{
    uint32_t position = hash_table_type::find_position(key);
    if (position == _capacity)
        position = hash_table_type::insert_force(value_type(key, mapped_type()));
    return _buckets[position].value.second;
}
                                                                 ...
                                                                 ValueType&& value)
{
    uint32_t position = hash_table_type::find_position(key);
    if (position == _capacity)
        hash_table_type::insert_force(value_type(key, std::forward<ValueType>(value)));
    else
        _buckets[position].value.second = std::forward<ValueType>(value);
}
